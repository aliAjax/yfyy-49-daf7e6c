...
router.get('/display/screen', (req, res) => {
  ...
  const { wait_limit = 10 } = req.query as any;
  ...
  stats.forEach((s: any) => {
    statsResult[s.status] = s.count;
    statsResult.total += s.count;
  });

  const windowStatus = db.prepare(`
    SELECT 
      w.id,
      w.name,
      w.number,
      ...
      si.name as service_item_name
    FROM windows w
    LEFT JOIN tickets t ON t.window_id = w.id AND t.status IN ('calling', 'processing')
    LEFT JOIN service_items si ON t.service_item_id = si.id
    WHERE w.status = 'open'
  ...
  windowStatus.forEach((item: any) => {
    if (!windowMap.has(item.id)) {
      windowMap.set(item.id, {
        id: item.id,
        name: item.name,
        number: item.number,
        status: item.status,
        current_ticket: item.ticket_number ? {
      ...
      windows.push(windowMap.get(item.id));
    }
  });
...
router.post('/issue', requireRoles('window', 'admin'), (req: AuthRequest, res) => {
  ...
  db.prepare(`
    ...
    VALUES (?, ?, ?, '现场取号', '叫号办理', ?)
  `).run(uuidv4(), req.user!.id, req.user!.name, `取号：${ticketNumber}，事项：${serviceItem.name}，申请人：${applicant_name || '现场群众'}`);
...
router.post('/:id/call', requireRoles('window', 'admin'), (req: AuthRequest, res) => {
  ...
  db.prepare(`
    INSERT INTO operation_logs (id, user_id, user_name, action, module, detail)
    VALUES (?, ?, ?, '呼叫', '叫号办理', ?)
  `).run(uuidv4(), req.user!.id, req.user!.name, `呼叫号票：${ticket.ticket_number}`);
...
router.post('/:id/start', requireRoles('window', 'admin'), (req: AuthRequest, res) => {
  ...
  db.prepare(`
    INSERT INTO operation_logs (id, user_id, user_name, action, module, detail)
    VALUES (?, ?, ?, '开始办理', '叫号办理', ?)
  `).run(uuidv4(), req.user!.id, req.user!.name, `开始办理号票：${ticket.ticket_number}`);
...
router.post('/:id/complete', requireRoles('window', 'admin'), (req: AuthRequest, res) => {
  ...
  db.prepare(`
    INSERT INTO operation_logs (id, user_id, user_name, action, module, detail)
    VALUES (?, ?, ?, '完成办理', '叫号办理', ?)
  `).run(uuidv4(), req.user!.id, req.user!.name, `完成办理号票：${ticket.ticket_number}`);
...
router.post('/:id/cancel', requireRoles('window', 'admin'), (req: AuthRequest, res) => {
  ...
  db.prepare(`
    INSERT INTO operation_logs (id, user_id, user_name, action, module, detail)
    VALUES (?, ?, ?, '过号取消', '叫号办理', ?)
  `).run(uuidv4(), req.user!.id, req.user!.name, `过号/取消号票：${ticket.ticket_number}`);
