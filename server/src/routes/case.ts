...
function requireApproverDepartment(req: AuthRequest, res: any): string | null {
  if (req.user!.role !== 'approver') {
    return null;
  }
  if (!req.user!.department_id) {
    res.status(403).json({ message: '未配置所属科室，无法访问催办记录' });
    return null;
  }
  return req.user!.department_id;
}

function canAccessWarningCase(req: AuthRequest, res: any, caseItem: any, action: string): boolean {
  if (req.user!.role !== 'approver') {
    return true;
  }
  const departmentId = requireApproverDepartment(req, res);
  if (!departmentId) {
    return false;
  }
  if (caseItem.department_id !== departmentId) {
    res.status(403).json({ message: `无权${action}其他科室的办件` });
    return false;
  }
  return true;
}
...
router.get('/warnings/overdue', requireRoles('approver', 'admin'), (req: AuthRequest, res) => {
  ...
  if (req.user!.role === 'approver') {
    const scopedDepartmentId = requireApproverDepartment(req, res);
...
router.get('/:id/urge-records', requireRoles('approver', 'admin'), (req: AuthRequest, res) => {
  ...
  if (!canAccessWarningCase(req, res, caseItem, '查看')) {
    return;
  }
...
router.get('/:id/urge-check', requireRoles('approver', 'admin'), (req: AuthRequest, res) => {
  ...
  if (!canAccessWarningCase(req, res, caseItem, '操作')) {
    return;
  }
...
router.post('/:id/urge', requireRoles('approver', 'admin'), (req: AuthRequest, res) => {
  ...
  if (!canAccessWarningCase(req, res, caseItem, '催办')) {
    return;
  }
  ...
  if (target_user_id) {
    ...
    if (target_department_id && targetUser.department_id !== target_department_id) {
      return res.status(400).json({ message: '催办对象不属于目标科室' });
    }
