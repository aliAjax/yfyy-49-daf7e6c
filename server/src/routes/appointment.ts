...
// 预约签到取号
router.post('/:id/check-in', requireRoles('window', 'admin'), (req: AuthRequest, res) => {
  const { id } = req.params;
  ...
  const today = dayjs().format('YYYY-MM-DD');
  const isExpired = dayjs(appointment.appointment_date).isBefore(today);

  if (isExpired && appointment.status !== 'completed') {
  ...
  if (existingTicket) {
    db.prepare(`
      ...
      VALUES (?, ?, ?, '预约签到取号（重复请求）', '预约', ?)
    `).run(uuidv4(), req.user!.id, req.user!.name, `已存在号票：${existingTicket.ticket_number}，事项：${existingTicket.service_item_name}，申请人：${appointment.applicant_name || '现场群众'}`);
  ...
  if (appointment.status === 'completed' && !existingTicket) {
    return res.status(400).json({ message: '该预约状态异常，请联系管理员' });
  }

  if (appointment.status === 'pending') {
    return res.status(400).json({ message: '该预约待确认，无法取号' });
  }

  if (appointment.status !== 'confirmed') {
    return res.status(400).json({ message: '该预约状态不支持取号' });
  }

  const serviceItem = db.prepare('SELECT * FROM service_items WHERE id = ? AND status = ?')
    .get(appointment.service_item_id, 'active') as any;
  ...
  const tx = db.transaction(() => {
    const doubleCheck = db.prepare('SELECT id FROM tickets WHERE appointment_id = ?').get(id);
    if (doubleCheck) {
    ...
    db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, 'waiting', ?, ?)
    `).run(ticketId, ticketNumber, appointment.service_item_id, appointment.user_id, id,
          appointment.applicant_name || '现场群众', appointment.applicant_phone || null);

    db.prepare("UPDATE appointments SET status = 'completed', updated_at = CURRENT_TIMESTAMP WHERE id = ? AND status = 'confirmed'")
      .run(id);
  });
  ...
  try {
    ...
    if (error.message === 'DUPLICATE_TICKET') {
  ...
  db.prepare(`
    INSERT INTO operation_logs (id, user_id, user_name, action, module, detail)
    VALUES (?, ?, ?, '预约签到取号', '预约', ?)
  `).run(uuidv4(), req.user!.id, req.user!.name, `预约签到取号：${ticketNumber}，事项：${serviceItem.name}，申请人：${appointment.applicant_name || '现场群众'}`);

  const existingNotification = db.prepare(
    'SELECT id FROM notifications WHERE user_id = ? AND related_id = ? AND sub_type = ?'
  ).get(appointment.user_id, id, 'appointment_checked_in');

  if (!existingNotification) {
    db.prepare(`
      INSERT INTO notifications (id, user_id, type, sub_type, title, content, related_id)
      VALUES (?, ?, 'appointment', 'appointment_checked_in', '预约签到取号成功', ?, ?)
    `).run(uuidv4(), appointment.user_id, `您的预约已签到取号，号票：${ticketNumber}，事项：${serviceItem.name}`, id);
  }
